...
#include <vector>
namespace avva::msg {
using BoundedId = std::string; using BoundedText255 = std::string;
...
struct HealthSummary {
  ...
  std::vector<ComponentHealthIssue> issues{};
  bool truncated{};
};
struct Warning {
  ReasonCode code{};
...
struct RunManifest {
  ...
  std::vector<SimInstanceProfile> sim_instances{};
  std::vector<BoundedId> required_sim_ids{};
  std::vector<EgoProfile> ego_profiles{};
  ...
  std::vector<SoftwareVersion> software_versions{};
...
struct WorldStateFrame {
  ...
  std::vector<BoundedId> expected_ego_ids{};
  std::vector<BoundedId> expected_traffic_actor_ids{};
...
struct EgoObservationFrame {
  ...
  std::vector<ObservedObject> gt_objects{};
  ControlSetMeta control_set_meta{};
...
struct NpcControlBatch {
  ...
  std::uint64_t target_tick_id{};
  ControlSetMeta control_set_meta{};
  std::vector<NpcControlItem> items{};
  std::vector<ActorLifecycleIntent> lifecycle_intents{};
...
struct CommandSetReady {
  ...
  Validity validity{};
  std::vector<BoundedId> missing_actor_ids{};
  std::vector<ActorReason> actor_reasons{};
